...
Coral persists these best-effort observations per workspace under its [local state directory](/getting-started/installation#local-state) at `workspaces/<workspace>/statistics/profile.json`. Values reflect the last local query that touched the table, may be sample-derived, and unsupported column types may stay `NULL`. Agents should treat them as hints unless `stats_precision` is `exact`.
