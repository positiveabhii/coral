...
| Tool     | `search_values`  | Search values learned from prior successful SQL results |
...
`search_values` accepts a required `term` plus optional `schema`, `table`, `column`, `limit`, and `offset` arguments. It searches fielded value candidates learned from prior successful `sql` calls and returns compact matches grouped by field, such as `{"field":"datadog.logs.service","values":["titaness-worker"],"total":1}`. Use it when the agent has a private clue but does not know which field can filter for it.
...
The agent should call `list_tables`, `search_tables`, or query `coral.tables` to return schemas from your installed sources, and use `describe_table` or `list_columns` for table-specific metadata. When the task includes a known value but the field is unclear, the agent can call `search_values` after some prior SQL results have been observed. Large catalogs and value searches should be paged with `limit` and `offset`.
