...
For discovery, have the agent query `coral.tables`, `coral.columns`, and `coral.inputs` over the `sql` tool. Include `coral.tables.guide` when inspecting table metadata. `coral.columns` is the canonical column metadata surface because it includes column descriptions and `is_required_filter`. `list_tables` is useful as a flat fully qualified table index, and its optional `schema` argument performs exact schema matching only. For quick table shape checks, agents can also run `DESCRIBE <schema>.<table>` or `SHOW COLUMNS FROM <schema>.<table>`.
