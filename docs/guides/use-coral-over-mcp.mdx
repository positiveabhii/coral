...
| Tool     | `feedback`                 | Store a local blocked-agent feedback report and upload an anonymous copy when enabled  |
...
`list_table_functions` returns source-scoped table functions in pages and accepts optional `schema`, `limit`, and `offset` arguments. Use the returned `sql_reference` with named arguments in `FROM`, for example `github.search_issues(q => 'repo:withcoral/coral deploy failure')`.
`search_table_functions` accepts a required `pattern` plus optional `schema`, `ignore_case`, `limit`, and `offset` arguments for deterministic regex matching over function names, descriptions, arguments, and result columns.
...
For richer metadata, have the agent query `coral.tables`, `coral.columns`, `coral.table_functions`, and `coral.inputs` over the `sql` tool instead of relying only on `list_tables` or `coral://tables`.
...
The agent should call `list_tables`, `search_tables`, `list_table_functions`, `search_table_functions`, or query `coral.tables` and `coral.table_functions` to return schemas from your installed sources, and use `describe_table` or `list_columns` for table-specific metadata. Large catalogs should be paged with `limit` and `offset`.
