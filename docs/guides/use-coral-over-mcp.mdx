...
For discovery, have the agent query `coral.tables`, `coral.columns`, and `coral.inputs` over the `sql` tool. `coral.tables.guide` carries per-table usage guidance; `coral.columns` is the canonical column metadata surface because it includes column descriptions, virtual-column metadata, and `is_required_filter`. `list_tables` is useful only as a flat fully qualified table index, and its optional `schema` argument performs exact schema matching. For quick table shape checks, agents can also run `DESCRIBE <schema>.<table>` or `SHOW COLUMNS FROM <schema>.<table>`.
