...
When Coral runs over MCP, each MCP protocol request produces a `coral.mcp.*` server span. Capability listing requests such as `tools/list` and `resources/list` are static and normally have no local gRPC children. Tool calls and resource reads that need live data emit `grpc.client.*` client spans for local Coral app calls, which parent the app's gRPC server handler spans. This keeps catalog discovery such as `list_catalog` and `coral://tables` attached to Coral-owned request spans without turning the long-running `mcp-stdio` process into one giant CLI trace.
