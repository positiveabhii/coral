...
A source spec tells Coral how to connect to an API or read a local dataset, what relations to expose, and what columns each relation has. Once added, the source works exactly like a bundled one: same SQL, same `coral.relations`, `coral.columns` and `coral.inputs`, same MCP surface.
...
5. Inspects `coral.relations`, `coral.columns` and `coral.inputs` to check the shape
6. Refines and repeats until the relations look right
...
3. Confirm the install-time validation output:
   - the source exposes the expected relations
...
4. Run real `coral sql` queries against the new relations to confirm the shape is useful beyond the validation checks.
...
For the full set of HTTP response, pagination, auth, and column fields, see [HTTP-backed relations](/reference/source-spec-reference#http-backed-relations).
...
- **Use `coral.relations`, `coral.columns`, and `coral.inputs`.** They show you exactly what Coral sees after adding a source — including required filters and column metadata.
