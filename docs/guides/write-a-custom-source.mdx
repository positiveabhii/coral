...
A source spec tells Coral how to connect to an API or read a local dataset, what tables and table functions to expose, and what columns each table has. Once added, the source works exactly like a bundled one: same SQL, same `coral.tables`, `coral.columns`, `coral.table_functions`, and `coral.inputs`, same MCP surface.
