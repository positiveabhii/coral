...
- **`coral`** — teaches your agent the discovery-first MCP workflow for answering data questions through Coral, including how to use `list_catalog`, `search_catalog`, `search_columns`, `coral.tables`, `coral.table_functions`, `coral.columns`, and `coral.inputs`.
