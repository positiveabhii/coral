...
| Tools           | `sql`, `list_catalog`, `search_catalog`, `describe_table`, `list_columns`, optional `feedback`, and optional Code Mode `exec`/`wait` |
| Resources       | `coral://guide`, `coral://tables`                                   |

Uses `coral-client` to reach `coral-app`, same transport as the CLI.
The finite Coral function bridge is owned by `coral-mcp` and is reused by both
ordinary MCP calls and nested Code Mode calls, so Code Mode does not bypass the
MCP surface or expose provider-specific tools.
