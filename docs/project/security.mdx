...
`list_columns`, optional `feedback`, optional Code Mode `exec`/`wait`,
`coral://guide`, and `coral://tables`.
When Code Mode is enabled, JavaScript runs in an isolated V8 runtime without
Node, filesystem, network, or console access. Nested Code Mode calls go through
the same finite Coral function bridge as direct MCP calls; provider-specific
tools, source tables, and backend API operations are not exposed as direct
