...
MCP surface: `sql`, `list_relations`, `search_relations`, `describe_relation`,
`list_columns`, `coral://guide`, and `coral://relations`.
...
- MCP clients can query installed sources, but they do not receive raw stored
  secret values through the discovery relations or resources.
