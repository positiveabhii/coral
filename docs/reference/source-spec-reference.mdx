...
only to the token endpoint, are never included in the authorization URL, and may
be stored as internal credential metadata so Coral can refresh the access token
