...
Each query opens a fresh MCP session: stdio sources spawn a child process per
`call_tool`, and Streamable HTTP sources run the initialize handshake on every
call. Session pooling is a future optimization.

Query-time failures surface as one of the following structured error reasons,
each carrying source, relation, tool, and stage metadata:
