...
`token_request` customizes the token exchange. `body_format` is `form` by
default, matching `application/x-www-form-urlencoded`; set it to `json` for
providers that require a JSON token body. `headers` is an optional list of
literal headers to include on the token request, for provider API-version
headers or similar token-endpoint requirements:
