...
| `dsl_version` | Coral source spec DSL version (currently `4`)      |
...
or more read-only SQL checks after the source validates and its relations become
queryable.
...
FROM coral.columns
WHERE schema_name = 'github' AND relation_name = 'issues'
...
For `jsonl` and `parquet`, each relation points at a file location:
...
- `response: {}` means "use the default response rules" for an HTTP relation
...
### HTTP Relation Writes

HTTP relations can declare write operations alongside the optional `read` shape.
Writes execute through SQL DML and are only sent when the physical plan runs,
so `EXPLAIN` remains side-effect free.
...
- `insert`, `update`, `delete`, and `truncate` are optional per relation.
- `{{input.*}}` reads values supplied by `INSERT` or `UPDATE`.
- `{{key.*}}` marks direct key filters required by `UPDATE` and `DELETE`; Coral derives the key columns from those templates.
- `UPDATE` and `DELETE` currently require direct equality filters for every derived key column.
- `INSERT` and `UPDATE` send a default JSON body from writable input columns unless the operation declares an explicit request body.
- Query `coral.relations`, `coral.columns`, and `coral.functions` to inspect read/write capability metadata.
