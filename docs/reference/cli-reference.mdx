...
```shellscript
coral mcp-stdio [--enable-feedback] [--enable-code-mode] [--code-mode-only]
```
...
| Tool     | `exec`           | Run Code Mode JavaScript when the binary is built with Code Mode and `--enable-code-mode` is set |
| Tool     | `wait`           | Resume or terminate Code Mode cells when the binary is built with Code Mode and `--enable-code-mode` is set |
| Resource | `coral://guide`  | Usage guide for agents           |
| Resource | `coral://tables` | Table summaries for all sources  |

`sql` returns structured `{ columns, rows, row_count }` results and accepts optional bound `params` as either an array for `$1` placeholders or an object for named placeholders. `--code-mode-only` requires `--enable-code-mode` and advertises only `exec` and `wait`; the finite Coral functions remain callable inside Code Mode. Code Mode is a JavaScript orchestration layer around Coral SQL, not a replacement for SQL: relational work should usually stay in one `tools.sql` query with filtering, projection, `JOIN`s, grouping, ordering, and limits in SQL so DataFusion can optimize the plan. Code Mode `exec` returns `{ status, result }`; `result` is the script's returned JSON-serializable value. Bare final expressions and bare awaited tool calls are ignored unless returned. Returned object properties with `undefined` values are omitted, `undefined` array items become `null`, and top-level `undefined` means no returned value.
