...
Use an OAuth access token with the
`https://www.googleapis.com/auth/gmail.readonly` scope.

To connect interactively, choose "Connect with Google" and provide a
Google OAuth client ID and client secret when prompted. Gmail access
tokens expire; rerun `coral source add --interactive gmail` when the
stored token needs to be replaced.

To paste a token manually instead, obtain one from your own OAuth
flow with the same read-only Gmail scope.
