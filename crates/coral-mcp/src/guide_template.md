...
- `list_table_functions` shows source-scoped table functions in pages, including accepted arguments and result columns.
- `search_table_functions` searches function names, descriptions, arguments, and result columns with a Rust regex.
...
- `coral://tables` shows table summaries for all installed sources; `coral.tables`, `coral.columns`, `coral.table_functions`, and `coral.inputs` provide richer SQL metadata.
