...
- `search_values` searches fielded values learned from previous successful SQL results and returns compact matches grouped by field. Use it when you know a clue such as a service, pod, repo, channel, status, or error phrase but do not know which table field has that value.
