...
pub(super) fn completion_state(
    ...
    let Some(pending_promise) = pending_promise else {
        return match read_result_slot(scope) {
    ...
    match promise.state() {
        ...
        v8::PromiseState::Fulfilled => match read_result_slot(scope) {
            Ok(result) => CompletionState::Completed {
                stored_values,
                result,
                error_text: None,
...
fn read_result_slot(scope: &mut v8::PinScope<'_, '_>) -> Result<Option<JsonValue>, String> {
    let global = scope.get_current_context().global(scope);
    let Some(key) = v8::String::new(scope, "__coral_code_mode_result") else {
    ...
    structured_result_to_json(scope, value)
}
