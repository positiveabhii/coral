...
fn run_runtime(
    ...
    if let Err(error_text) = globals::install_globals(scope) {
        send_result(&event_tx, HashMap::new(), None, Some(error_text));
        return;
    }
    ...
    match module_loader::completion_state(scope, pending_promise.as_ref()) {
        CompletionState::Completed {
            ...
            send_result(&event_tx, stored_values, result, error_text);
            return;
        }
...
fn capture_scope_send_error(
    ...
    send_result(event_tx, stored_values, None, error_text);
}
