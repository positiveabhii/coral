...
impl TraceServiceApi for TraceService {
    async fn list_traces(
        ...
        let span = grpc_span(request.metadata(), "list_traces");
    ...
    async fn get_trace(
        ...
        let span = grpc_span(request.metadata(), "get_trace");
        let traces = self.traces.clone();
        async move {
            let request = request.into_inner();
...
fn trace_summary_to_proto(summary: TraceSummaryRecord) -> TraceSummary {
    TraceSummary {
        ...
        start_time_unix_nanos: summary.start_time_unix_nanos.to_string(),
        end_time_unix_nanos: summary.end_time_unix_nanos.to_string(),
        duration_nanos: summary.duration_nanos.to_string(),
...
fn trace_span_to_proto(span: TraceSpanRecord) -> TraceSpan {
    TraceSpan {
        ...
        end_time_unix_nanos: span.end_time_unix_nanos.to_string(),
