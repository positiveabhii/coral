...
async fn exchange_authorization_code(
    ...
    let mut request = http
        .post(&session.common.oauth.token_url)
        .header(ACCEPT, "application/json");
    for header in &session.common.oauth.token_request.headers {
    ...
    match (
        ...
        (Some(secret), Some(ManifestOAuthClientSecretTransport::RequestBody)) => {
            params.insert("client_id", session.common.client_id.clone());
    ...
    let request = match session.common.oauth.token_request.body_format {
